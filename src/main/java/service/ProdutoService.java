...
/*		List<Produto> produtos = produtoDAO.getByEstab(idEstab);
		...
		js += "let produtos = [";
		for (Produto p : produtos){
			js+= "{id: \""+p.getId()+"\", nome: \""+p.getNome()+"\", descricao: \""+p.getDescricao()+"\", categoria: \""+p.getCategoria()+"\", marca: \""+p.getMarca()+"\", unidade: \""+p.getUnidade()+"\", preco: \""+p.getPreco()+"\", imagem: \""+p.getImagem()+"\", supermercado: \""+p.getSupermercado()+"\"},";
		}
		js += "{}]";
		form = form.replaceFirst("VARIAVEL", js);
		return form; */
